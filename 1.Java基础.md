...
2. 下面将举例说明ART是如何实现使用内存屏障访问volatile变量，来保证有序性，由于目前常用的ART虚拟机默认是机器码执行模式，下面都是基于这种模式介绍：  
    ...
    .end method
    ```  
    - iget-XX/iput-XX Java指令会先编译成汇编指令，如果成员变量被volatile修饰，arm架构下会添加一条内存屏障指令，这样就实现内存顺序一致的要求，下面以iget指令生成对应内存屏障指令为例说明，在介绍synchronized时知道，ARM平台编译iget指令的函数在InstructionCodeGeneratorARMVIXL::HandleFieldGet（在art/compiler/optimizing/code_generator_arm_vixl.cc文件中）：  
    ```c
    void InstructionCodeGeneratorARMVIXL::HandleFieldGet(HInstruction* instruction,
      ...
      if (is_volatile) {
        if (load_type == DataType::Type::kReference) {
          ...
          // 对于示例代码中读取boolean类型的成员变量，最终会走到这里，添加内存屏障指令
    ...
    ```  
    - 可以看到，最终生成了dmb汇编指令（和Hotspot虚拟机生成的指令不同），并且传入了ISH，dmb是数据内存屏障，确保程序指令执行顺序不会乱掉，ISH表示内部共享区域，也就是说之后的指令会操作共享区域的数据，因此修改完毕之后，需要及时通知其他 CPU；
    - ART虚拟机并不是直接执行dex文件，而是执行优化好的二进制代码（放在.oat文件中），【TODO】  
4. ART虚拟机如果是解释执行模式，Java层想要读取或设置某个对象的成员变量，最终会调用到C层Object对象的GetField()函数和SetField()函数；  


10. volatile变量和普通变量的区别：  
    1）读操作没区别；  
