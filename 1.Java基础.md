...
3. 简单了解一下Java线程的内存模型：  
    - CPU硬件结构如图所示：  
    ![CPU硬件结构](https://github.com/JQLewis/AndroidNotes/blob/master/notes/iv/1-pics/CPU%E7%A1%AC%E4%BB%B6%E7%BB%93%E6%9E%84.png?raw=true)  
    ...
    ![Java线程内存模型](https://github.com/JQLewis/AndroidNotes/blob/master/notes/iv/1-pics/Java%E7%BA%BF%E7%A8%8B%E6%A8%A1%E5%9E%8B.png?raw=true)  
...
2. 在ART虚拟机中，关于线程同步涉及到的类如下图所示，真正实现线程同步功能的是Monitor类（art/runtime/monitor.cc文件中定义）；  
    ![线程同步相关类](https://github.com/JQLewis/AndroidNotes/blob/master/notes/iv/1-pics/ART%E4%B8%AD%E7%BA%BF%E7%A8%8B%E5%90%8C%E6%AD%A5%E7%9B%B8%E5%85%B3%E7%B1%BB.png?raw=true)  
...
9. **解释执行模式**：Android 11 的代码执行入口会走到MONITOR_ENTER()方法（runtime/interpreter/interpreter_switch_impl-inl.h文件中），接着走到静态方法DoMonitorEnter()中（在runtime/interpreter/interpreter_common.h文件）：  
    ...
    ```  
    总结如下：  
    ![MonitorEnter实现](https://github.com/JQLewis/AndroidNotes/blob/master/notes/iv/1-pics/MonitorEnter%E5%AE%9E%E7%8E%B0.png?raw=true)  
...
Java提供了种类丰富的锁，按照特性的不同，可以按以下宏观概念理解：  
    ...
    - 可重入锁和非可重入锁；  
    - 独享锁（排他锁）和共享锁；  
    ![Java锁的种类](https://github.com/JQLewis/AndroidNotes/blob/master/notes/iv/1-pics/Java%E9%94%81%E7%9A%84%E7%A7%8D%E7%B1%BB.png?raw=true)  
...
3. 说白了，自旋锁可以简单理解为：如果发现锁被其他线程拿着，在当前线程，一定条件的while循环，而不是阻塞当前线程：  
    ![自旋锁](https://github.com/JQLewis/AndroidNotes/blob/master/notes/iv/1-pics/Java%E8%87%AA%E6%97%8B%E9%94%81.png?raw=true)  
...
3. Java中具体的实现，ReentrantLock可以选择创建哪种锁：  
    ...
    ![公平锁](https://github.com/JQLewis/AndroidNotes/blob/master/notes/iv/1-pics/Java%E5%85%AC%E5%B9%B3%E9%94%81.png?raw=true)
...
2. ReentrantLock和synchronized都是可重入锁，重入状态内部通过一个被volatile修饰的int类型的变量state来描述，仅对于当前线程来说，每次尝试获取锁成功时，state++，对应释放锁的state--：  
    ![可重入锁](https://github.com/JQLewis/AndroidNotes/blob/master/notes/iv/1-pics/Java%E5%8F%AF%E9%87%8D%E5%85%A5%E9%94%81.png?raw=true)  
...
- 具体的实现类是java.util.concurrent.locks.ReentrantReadWriteLock：  
    ![独享锁](https://github.com/JQLewis/AndroidNotes/blob/master/notes/iv/1-pics/Java%E7%8B%AC%E4%BA%AB%E9%94%81%E5%85%B1%E4%BA%AB%E9%94%81.png?raw=true)  
...
1. 主要功能如下：  
    ![Unsafe](https://github.com/JQLewis/AndroidNotes/blob/master/notes/iv/1-pics/Java%E9%AD%94%E6%B3%95%E7%B1%BBUnsafe.png?raw=true)  
...
#### 3.3.1 整型
1. Java整型，允许有负数，整型的范围具有平台无关性；
2. 有4种：  

    | 类型 | 存储需求 | 取值范围 |  
    | ------ | ------ | ------ |  
    |int|4字节|-2147483648\~214748647|  
    |short|2字节|-32768\~32767|  
    |long|8字节|-9223372036854775808\~9223372036854775807|  
    |byte|1字节|-128\~127|  
3. Java没有任何无符号(unsigned)形式的int、short、long、byte类型；
4. 由于八进制是0开头，容易混淆，所以不建议使用八进制表示数字，一般使用十六进制较多，前缀0x，例如0xCAFF；
...
2. Java程序中，很多情况下float类型精度无法满足需求，所以一般使用double类型；
3. 可以使用十六进制表示浮点数，例如：0.125=2^-3，表示为0x1.0p-3，p表示指数而不是e，尾数采用十六进制，指数采用十进制，基数是2不是10；
4. **判断一个值是否是非数值**：Double.isNaN(x)，return true表示x不是一个数；
5. 浮点数值采用二进制系统表示，在二进制系统中无法精确表示1/10(就像十进制无法精确表示1/3一样)，所以浮点数(2.1-1.1)打印结果为0.899999999999，而不是0.9；
6. 如果不允许有任何舍入误差，可以使用**BidDecimal类**；


#### 3.3.3 char类型
1. char类型的字面量值必须用单引号，例如'A'是编码值为65所对应的字符常量，而"A"是包含一个字符A的字符串；
2. char类型的值可以表示为十六进制值，范围： \u0000 ~ \Uffff，\u表示转义序列(也有其他转义序列)，例如\u03C0表示希腊字母π；
3. 特殊字符的转义序列：
    | 转义序列 | 名称 | Unicode值 |  
    | ------ | ------ | ------ |  
    |\b|退格|\u0008|
...
1. 字符编码历史：
    1. 最开始，计算机只在美国使用，他们定义8位为1个字节(2的8次方=256种状态)，用于表示不同的状态，把0×20以下的字节状态称为"控制码"，又将所有的空格、标点符号、数字、大小写字母分别用连续的字节状态表示，一直编到了第127号，这样计算机就可以存储英文了；
    2. 后来把这个方案叫做 ANSI 的”Ascii”编码（American Standard Code for Information Interchange，美国信息互换标准代码）。当时世界上所有的计算机都用同样的ASCII方案来保存英文文字格、标点符号、数字、大小写字母分别用连续的字节状态表示，一直编到了第127号，这样计算机就可以存储英文了；
    3. 后来世界各地开始使用计算机，但其他国家的字母不在ASCII中，所以又对127~256进行定义，称为"扩展字符集"；
    4. 中国规定：一个小于127的字符的意义与原来相同，但两个大于127的字符连在一起时，就表示一个汉字，前面的一个字节（他称之为高字节）从0xA1用到0xF7，后面一个字节（低字节）从0xA1到0xFE，又把数学符号、罗马希腊的字母、日文的假名们都编进去了，于是就把这种汉字方案叫做 "GB2312"。GB2312 是对 ASCII 的中文扩展；
    5. 到后来发现依然不能满足中国的需求，所以对GB2312扩展，扩展之后的编码方案被称为 GBK 标准，GBK包括了GB2312 的所有内容，同时又增加了近20000个新的汉字（包括繁体字）和符号；
    6. 后来少数民族也要用电脑了，于是我们再扩展，又加了几千个新的少数民族的字，GBK扩成了 GB18030；
    7. ISO（国际标谁化组织）的国际组织决定着手解决这个问题。他们采用的方法很简单：废了所有的地区性编码方案，重新搞一个包括了地球上所有文化、所有字母和符号 的编码！他们打算叫它"Universal Multiple-Octet Coded Character Set"，简称 UCS, 俗称 "unicode"；
    8. unicode在很长一段时间内无法推广，直到互联网的出现，为解决unicode如何在网络上传输的问题，于是面向传输的众多 UTF（UCS Transfer Format）标准出现了，顾名思义，UTF-8就是每次8个位传输数据，而UTF-16就是每次16个位；
    9. UTF-8最大的一个特点，就是它是一种变长的编码方式。它可以使用1~4个字节表示一个符号，根据不同的符号而变化字节长度，当字符在ASCII码的范围时，就用一个字节表示，保留了ASCII字符一个字节的编码做为它的一部分，注意的是unicode一个中文字符占2个字节，而UTF-8一个中文字符占3个字节）。从unicode到utf-8并不是直接的对应，而是要过一些算法和规则来转换；
2. Unicode出现之前已经有多种不同的编码标准：美国的ASCII、西欧的ISO 8859-1、中国的GB 18030和BIG-5等；
3. 不同的编码标准带来2个问题，而Unicode编码机制就是解决以下2个问题：
    - 给定的一个代码值，在不同编码标准下可能对应不同的字母；
    - 编码长度可能不同，有些字符采用单字节编码，有些可能采用2~N个字节编码；
4. Java中，char类型描述了UTF-16编码中的一个代码单元(即每个字符用16位表示)；
5. 强烈建议不要使用char类型；


#### 3.3.5 boolean类型
1. 在C++中，值0相当于false，非0值相当于true；
...
1. 种类：
    ...
    |&|位与|有0则0|  
    |||位或|有1则1|  
    |^|位异或|相同则0，不同则1；一个数据对另一个数据位异或两次，该数本身不变|  
    |~|取反|按位取反|  
    |>>|向右移|最高位是0，左边补齐0;最高为是1，左边补齐1|  
    |>>>|无符号右移|无论最高位是0还是1，左边补齐0|  
    |<<|向左移|左边最高位丢弃，右边补齐0|  
2. 交换两个数字，且不会遇到溢出，用加减法，遇到超出int范围的数，会损失精度：
    ```java
    var a = 20
    var b = 10
    a = a xor b // a = 30 
...
### 3.9 大数值
1. 如果基本的正数和浮点数无法满足需求，可以使用java.math包下的BigInteger(大正数)和BidDecimal(大浮点数)；
2. 这两个类可处理包含任意长度数字序列的数；
3. 该类提供了计算和、差、积、商的方法接口：
    ```java
    public BigDecimal add(BigDecimal augend);
    public BigDecimal subtract(BigDecimal subtrahend);
    public BigDecimal multiply(BigDecimal multiplicand);
    public BigDecimal divide(BigDecimal divisor, int roundingMode); // roundingMode舍入方式，java.math.RoundingMode#HALF_UP即为四舍五入
    ```
