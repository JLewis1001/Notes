...
- 对于获取锁失败后，肯定存在某种排队等待的机制，让线程继续拥有获得锁的机会，而不是直接结束流程；从上面代码可以知道，需要看下acquire()方法的实现逻辑，而该方法定义在java.util.concurrent.locks.AbstractQueuedSynchronizer中，所以下面分析一下AQS的原理；  
- AQS的整体结构如下图所示：  
