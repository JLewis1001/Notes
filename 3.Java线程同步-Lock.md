...
### 3.5 可重入锁 VS 非可重入锁
1. 以下代码不会造成死锁，因为synchronized这个内部锁是可重入锁，在执行t2()时发现是同一个线程再次想获取锁，这时会直接拿到，而不用等t1()释放锁后才去拿，但如果相同的代码，使用不可重入锁则会发生死锁的情况：  
    ```java
    ...
    ```
2. synchronized是可重入锁，之前了解过，ART虚拟机synchronized的可重入状态封装在c++代码LockWord类的成员变量uint32_t value_中，如果是瘦锁，该value_字段的27-16位就保存了Lock Count；
3. ReentrantLock也是可重入锁，重入状态通过一个被volatile修饰的int类型的变量state来描述，仅对于当前线程来说，每次尝试获取锁成功时，state++，对应释放锁的state--：  
