    ...
    bool ClassLinker::LoadSuperAndInterfaces(Handle<mirror::Class> klass, const DexFile& dex_file) {
      ...
      if (interfaces != nullptr) {
        for (size_t i = 0; i < interfaces->Size(); i++) {
          ...
          // 根据方法参数可知：通过type_idx从dex文件中找到这个类型的字符串描述符，然后通过字符串描述符找到对应的类的Class对象
      ...
      // 【TODO】如果父类和接口类能正常加载？父类和接口类加载完后放到了哪里？需要查看LoadClass()和LinkClass()
...
8. 继续看【6】，ClassLinker::DoResolveType()代码：  
    ...
    ObjPtr<mirror::Class> ClassLinker::DoResolveType(dex::TypeIndex type_idx,
      ...
      // 通过描述符又执行FindClass()方法去查询接口类，又走一遍 Find->Define 的流程
