...
2. 再看ClassLoader.loadClass()：  
    ...
    static jclass VMClassLoader_findLoadedClass(JNIEnv* env, jclass, jobject javaLoader,
                                            jstring javaName) {
      ...
      // 其内部调用了ClassLinker::LookupClass(self, descriptor, hash, class_loader)，内部就是从内存（ClassTable，和ClassLoader对应）的Class集合中查找，如过找到
...
5. 接着看ClassLinker::FindClass()方法，实现在art/runtime/class_linker.cc文件中，下面代码只关注装载单个类的部分，数组暂不介绍：  
    ...
    ObjPtr<mirror::Class> ClassLinker::FindClass(Thread* self,
      ...
      // 此处说明该类没有被装载过，我们Java层传入的class_loader对象是PathClassLoader，所以不走这里
    ...
    ```  
6. ClassLinker::FindClassInBaseDexClassLoader()内部的实现，就是类加载中双亲委托机制的具体代码实现，先从当前类加载器的父亲开始寻找，如果在双亲类加载器中没找到，才尝试自己装载该类；根据此处的上下文加载Foo类，遍历完双亲类加载器发现都没有加载过，就会自己尝试从DexFile集合中查找该类是否有定义，即会走到ClassLinker::FindClassInBaseDexClassLoaderClassPath()方法中；  
7. ClassLinker::FindClassInBaseDexClassLoaderClassPath()内部逻辑简单理解就是：遍历当前ClassLoader加载的DexFile集合，通过类的描述符找到类对应的ClassDef对象，接着执行DefineClass()方法，该方法是真正装载类的入口方法；  
    > 
9. 如果【当前类】的描述符信息可以找到，就会执行DefineClass()方法，该方法将开始从dex文件中加载这个类，其代码如下：  
    ```c
