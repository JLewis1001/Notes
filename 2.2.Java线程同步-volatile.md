...
### 2.4 内存屏障
1. volatile是通过编译器在生成字节码时，在指令序列中增加【内存屏障】指令实现可见性和有序性的，对于 HotSpot JVM 的具体实现中，描述了基于JSR-133规范定义的4种基本内存屏障指令：  
...
- 上述是内存屏障的定义是HotSpot JVM 中描述的，但不同的平台具体实现也会有所不同，下面将举例说明ART是如何实现使用内存屏障访问volatile变量，来保证有序性和可见性，由于目前常用的ART虚拟机默认是机器码执行模式，下面都是基于这种模式介绍；  
