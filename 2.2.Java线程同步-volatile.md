...
- dmb指令是什么？查看ARM官方解释：[DMB官方解释](https://developer.arm.com/documentation/dui0489/c/arm-and-thumb-instructions/miscellaneous-instructions/dmb--dsb--and-isb)   
    ...
    > 1. dmb 是一个数据内存屏障；  
    > 2. dmb 确保当前程序指令执行顺序不会有重排序，不影响其他指令；  
- dmb指令后面紧跟着ish指令，ish指令又代表什么？表示内部共享区域，也就是说之后的指令会操作共享区域的数据（从JVM层面看，就是访问堆内存中的数据，成员变量），因此修改完毕之后，需要及时通知其他CPU，即实现了可见性；  
- 总结：【volatile通过dmb指令实现有序性，ish实现可见性】；  
