...
#### 2.5.3 Android虚拟机实现
- iget-XX/iput-XX Java指令会先编译成汇编指令，如果成员变量被volatile修饰，ARM架构下会添加一条内存屏障指令，以iget为例，ARM平台编译iget指令时，会在iget后生成一条内存屏障指令，目的是保证执行iget操作获取某成员变量的操作不会被虚拟机指令重排序优化放到内存屏障指令后执行，这样来实现内存顺序一致的要求。下面以iget指令生成对应内存屏障指令为例说明，在介绍synchronized时知道，ARM平台编译iget指令的函数在InstructionCodeGeneratorARMVIXL::HandleFieldGet（在art/compiler/optimizing/code_generator_arm_vixl.cc文件中）：  
    ```c
