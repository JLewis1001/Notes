...
### 2.4 内存屏障
1. volatile是通过编译器在生成字节码时，在指令序列中增加【内存屏障】指令实现可见性和有序性的，下面是JSR-133规范中定义的4种基本内存屏障指令：  
...
### 2.5 volatile的实现
下面将举例说明ART是如何实现使用内存屏障访问volatile变量，来保证有序性，由于目前常用的ART虚拟机默认是机器码执行模式，下面都是基于这种模式介绍：  
...
- ART虚拟机并不是直接执行dex文件，而是执行优化好的二进制代码（放在.oat文件中），下面是如何编译查看.oat文件的流程：  
    ...
    ```c
      ...
      2: void juc.VolatileFoo.setFlag(boolean) (dex_method_idx=3)
    DEX CODE:
      0x0000: 5c01 0000                 | iput-boolean v1, v0, Z juc.VolatileFoo.flag // field@0
      0x0002: 0e00                      | return-void
    OatMethodOffsets (offset=0x00000820)
      code_offset: 0x00001059 
    OatQuickMethodHeader (offset=0x00001040)
      vmap_table: (offset=0x00000834)
        Optimized CodeInfo (number_of_dex_registers=2, number_of_stack_maps=0)
          StackMapEncoding (native_pc_bit_offset=0, dex_pc_bit_offset=0, dex_register_map_bit_offset=1, inline_info_bit_offset=1, register_mask_bit_offset=1, stack_mask_index_bit_offset=1, total_bit_size=1)
          DexRegisterLocationCatalog (number_of_entries=0, size_in_bytes=0)
    QuickMethodFrameInfo
      ...
      vr_stack_locations:
        ins: v0[sp + #4] v1[sp + #8]
        method*: v2[sp + #0]
    CODE: (code_offset=0x00001059 size_offset=0x00001054 size=12)...
      0x00001058: f3bf8f5b      dmb ish
      0x0000105c: 720a          strb r2, [r1, #8]
      0x0000105e: f3bf8f5b      dmb ish
      0x00001062: 4770          bx lr
    ```  
