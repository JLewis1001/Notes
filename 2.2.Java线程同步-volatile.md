...
### 2.4 内存屏障
1. 本节引入内存屏障的概念，但是是基于HotSpot JVM进行描述，了解即可，ART有对应的概念，但具体的实现和HotSpot不同，后续介绍；  
2. volatile是通过编译器在生成字节码时，在指令序列中增加【内存屏障】指令实现可见性和有序性的，对于 HotSpot JVM 的具体实现中，描述了基于JSR-133规范定义的4种基本内存屏障指令：  
    ...
    - LoadStore：类推；  
    - StoreLoad：类推；  
3. JVM层面，会在volatile读写操作的前后，都加上内存屏障指令，读操作和写操作所添加的内存屏障指令规定如下所示：  
    ```java
    LoadLoadBarrier指令
    volatile 读操作
    LoadStoreBarrier指令
    ...
    StoreLoadBarrier指令
    ```  
4. 在 JVM 层面通过如下定义的四种内存屏障操作规范了 volatile 的读、写任意组合都是有序的：  
...
- 上述内存屏障的定义是HotSpot JVM 中描述的，但不同的虚拟机具体实现会有所不同，下面将举例说明ART是如何实现使用内存屏障访问volatile变量，来保证有序性和可见性，由于目前常用的ART虚拟机默认是机器码执行模式，下面都是基于这种模式介绍；  
...
- 继续查看反编译后的VolatileFoo.class文件：  
    ...
    ```  
- 在Java类中，类及成员变量、成员函数都有访问修饰符，这些访问控制信息会通过转换为对应的access_flags，在字节码中，被volatile修饰的成员变量会增加ACC_VOLATILE标记，后续会根据这个标记位和CPU特性，在读/写成员变量的前后，分别生成内存屏障指令，来保证读/写成员变量时的有序性；  
...
- iget-XX/iput-XX Java指令会先编译成汇编指令，如果成员变量被volatile修饰，ARM架构下会添加一条内存屏障指令，以iget为例，ARM平台编译iget指令时，会在iget前面生成一条内存屏障指令，目的是保证执行iget操作获取某成员变量前的操作不会被虚拟机指令重排序优化放到iget后执行，这样就实现内存顺序一致的要求。下面以iget指令生成对应内存屏障指令为例说明，在介绍synchronized时知道，ARM平台编译iget指令的函数在InstructionCodeGeneratorARMVIXL::HandleFieldGet（在art/compiler/optimizing/code_generator_arm_vixl.cc文件中）：  
    ...
    ```  
- 可以看到，最终生成了dmb汇编指令（和Hotspot虚拟机生成的指令不同），并且传入了ISH，dmb是数据内存屏障，该指令确保程序指令执行顺序不会乱掉；  
...
- 具体内容如下所示，截取部分重要内容：  
    ...
    CODE: (code_offset=0x00001039 size_offset=0x00001034 size=26)...
      ...
      0x0000104c: f3bf8f5b      dmb ish  // 写操作：此处是写flag变量后，插入保证有序性的指令
...
- dmb指令是什么？查看ARM官方解释：[DMB官方解释](https://developer.arm.com/documentation/dui0489/c/arm-and-thumb-instructions/miscellaneous-instructions/dmb--dsb--and-isb)   
    ...
    > 1. dmb 是一个数据内存屏障；  
    > 2. dmb 仅当所有在它前面的存储器访问操作都执行完毕后，才提交(commit)在它后面的存储器访问操作；即，确保当前程序指令执行顺序不会有重排序，且不影响其他指令；  
- ish（Inner Shareable）指令又代表什么？表示访问内部共享区域，也就是说之后的指令会操作共享区域的数据（从JVM层面看，就是访问堆内存中的数据、成员变量），因此修改数据完毕后，会及时通知其他CPU，即实现了可见性；  
- 总结：ARM平台，【通过dmb指令实现volatile的有序性，ish指令实现可见性】；  
