...
- 上述是内存屏障的定义是HotSpot JVM 中描述的，但不同的虚拟机具体实现会有所不同，下面将举例说明ART是如何实现使用内存屏障访问volatile变量，来保证有序性和可见性，由于目前常用的ART虚拟机默认是机器码执行模式，下面都是基于这种模式介绍；  
- 按照分析synchronized的套路继续分析volatile：**Java层实现**、**字节码实现**、**虚拟机实现**、**CPU汇编指令实现**；  
